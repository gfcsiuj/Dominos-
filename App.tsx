...
const App: React.FC = () => {
  const [currentStep, setCurrentStep] = useState<AppState>('WELCOME');
  ...
  const startGame = () => {
    setTeam1({ name: settings.team1Name || 'فريقنا', scores: [], total: 0 });
    setTeam2({ name: settings.team2Name || 'فريقهم', scores: [], total: 0 });
    setCurrentStep('GAME');
      ...
      <AnimatePresence mode="wait">
        {currentStep === 'WELCOME' && (
          ...
          >
            <MotionDiv 
              ...
              className="w-40 h-40 md:w-56 md:h-56 bg-blue-600 dark:bg-blue-500 rounded-[3.5rem] mb-10 flex items-center justify-center shadow-2xl"
            >
              <div className="grid grid-cols-2 gap-3 p-4">
                {[1,2,3,4,5,6].map(i => <div key={i} className="w-3 h-3 bg-white rounded-full"></div>)}
              </div>
            </MotionDiv>
            <h1 className="text-6xl md:text-8xl font-black mb-4 tracking-tighter">دومينو</h1>
          ...
          >
            ...
            <div className="space-y-6 flex-1">
              <div className="space-y-2">
                ...
                <input
                  type="text"
                  placeholder="اسم فريقكم"
                  value={settings.team1Name}
                  onChange={e => setSettings({...settings, team1Name: e.target.value})}
                  className="w-full bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-3xl p-5 text-2xl font-bold text-right outline-none focus:border-blue-500 transition-all"
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="pt-6 text-right">
                ...
                <div className="flex gap-2">
                  {LIMIT_OPTIONS.map(opt => (
                    <button
                      key={opt}
                      onClick={() => setSettings({...settings, limit: opt})}
                      className={`flex-1 py-5 rounded-3xl text-3xl font-black transition-all border-2 ${
                        settings.limit === opt 
            ...
            <button
              onClick={startGame}
              className="bg-blue-600 text-white text-3xl font-black py-7 rounded-[2.5rem] shadow-2xl mt-8 mb-8 active:scale-95 transition-all"
            >
              يا الله
            </button>
          </MotionDiv>
        ...
        {currentStep === 'GAME' && (
          <MotionDiv key="game" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col relative bg-white dark:bg-slate-950">
            <div className="absolute top-0 left-0 w-full z-20 p-6 pt-12 flex justify-between items-center bg-gradient-to-b from-white/80 dark:from-slate-950/80 to-transparent">
               <button onClick={() => setCurrentStep('SETUP')} className="glass p-3 rounded-2xl dark:text-white text-slate-800"><RotateCcw className="w-6 h-6"/></button>
               <div className="glass px-6 py-2 rounded-full font-black text-sm tracking-widest text-blue-600 dark:text-blue-400">الهدف: {settings.limit}</div>
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div 
                ...
                className="flex-1 relative overflow-hidden bg-blue-50/20 dark:bg-blue-900/10 flex flex-col items-center justify-center border-b border-slate-100 dark:border-white/5 active:bg-blue-100/40 dark:active:bg-blue-900/20 transition-all duration-300"
              >
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-[0.03] dark:opacity-[0.07] font-black italic whitespace-nowrap fluid-bg-text pointer-events-none select-none text-blue-900 dark:text-blue-100">
                  {team1.name}
                </div>
                <MotionSpan key={team1.total} initial={{scale:0.8}} animate={{scale:1}} className="font-black leading-none drop-shadow-sm text-blue-600 dark:text-blue-400 fluid-score z-10">{team1.total}</MotionSpan>
                ...
                <div className="absolute bottom-6 left-6 flex gap-2 overflow-x-auto max-w-[60%] scrollbar-hide">
                    {team1.scores.slice(-4).map((s, i) => <span key={i} className="text-blue-600 dark:text-blue-300 text-sm font-black bg-white/80 dark:bg-blue-800/40 px-3 py-1.5 rounded-xl border border-blue-100 dark:border-blue-700/50">+{s}</span>)}
                </div>
              ...
              <div 
                ...
                className="flex-1 relative overflow-hidden bg-rose-50/20 dark:bg-rose-900/10 flex flex-col items-center justify-center active:bg-rose-100/40 dark:active:bg-rose-900/20 transition-all duration-300"
              >
                ...
                <MotionSpan key={team2.total} initial={{scale:0.8}} animate={{scale:1}} className="font-black leading-none drop-shadow-sm text-rose-600 dark:text-rose-400 fluid-score z-10">{team2.total}</MotionSpan>
                <span className="text-rose-800 dark:text-rose-200 font-black text-2xl uppercase tracking-widest z-10">{team2.name}</span>
                <div className="absolute bottom-6 left-6 flex gap-2 overflow-x-auto max-w-[60%] scrollbar-hide">
                    {team2.scores.slice(-4).map((s, i) => <span key={i} className="text-rose-600 dark:text-rose-300 text-sm font-black bg-white/80 dark:bg-rose-800/40 px-3 py-1.5 rounded-xl border border-rose-100 dark:border-rose-700/50">+{s}</span>)}
                </div>
            ...
            <AnimatePresence>
                {activeKeypad && (
                    ...
                    >
                        <div className="absolute inset-0 bg-slate-900/20 dark:bg-black/60 backdrop-blur-sm" onClick={() => setActiveKeypad(null)}></div>
                        <div className="relative bg-white dark:bg-slate-900 rounded-t-[3.5rem] p-6 pb-12 shadow-2xl border-t border-slate-100 dark:border-white/10">
                            <div className="flex justify-between items-center mb-8 px-4">
                                <div className="flex flex-col">
                                    <span className={`text-lg font-bold ${activeKeypad === 1 ? 'text-blue-500' : 'text-rose-500'}`}>
                                        نقاط {activeKeypad === 1 ? team1.name : team2.name}
                                    </span>
                                </div>
                                <div className={`text-6xl font-black tabular-nums ${keypadValue ? 'text-slate-900 dark:text-white' : 'text-slate-200 dark:text-slate-700'}`}>
                                    {keypadValue || '0'}
                                </div>
                            </div>
                                ...
                                {[1, 2, 3, 4, 5, 6, 7, 8, 9, 'C', 0, 'DEL'].map((k) => (
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            else if (k === 'DEL') setKeypadValue(prev => prev.slice(0, -1));
                                            else if (keypadValue.length < 3) setKeypadValue(prev => prev + k);
                                        }}
                                        className="h-16 md:h-20 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-2xl text-3xl font-black active:bg-blue-600 active:text-white transition-all flex items-center justify-center border border-slate-100 dark:border-transparent"
                                    >
                                        {k === 'DEL' ? <Delete className="w-7 h-7"/> : k}
                                    </button>
                                ))}
                            </div>
                            ...
                            <button
                                ...
                                className={`w-full py-6 rounded-[2rem] text-2xl font-black shadow-xl transition-all active:scale-95 ${
                                    activeKeypad === 1 ? 'bg-blue-600 text-white' : 'bg-rose-600 text-white'
                                }`}
                            >
                                {keypadValue ? 'تأكيد' : 'إلغاء'}
                            </button>
                        </div>
                    </MotionDiv>
                )}
        ...
        {currentStep === 'WINNER' && (
          ...
          >
            ...
            <MotionDiv
                ...
                className="w-40 h-40 bg-yellow-400 dark:bg-yellow-500 rounded-[3.5rem] mb-10 flex items-center justify-center shadow-2xl relative flex-shrink-0"
            >
                <Trophy className="w-24 h-24 text-white drop-shadow-lg" />
            </MotionDiv>
            
            <h2 className="text-xl font-bold text-slate-400 mb-2">البطل هو</h2>
            <h3 className="text-5xl md:text-7xl font-black mb-4 text-slate-900 dark:text-white text-center leading-tight">{winner?.name}</h3>
            
            <p className="text-2xl font-black text-blue-600 dark:text-blue-400 text-center mb-10">{winner?.message}</p>

            <div className="w-full max-w-sm bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] p-6 mb-12 border border-slate-100 dark:border-white/5 flex flex-col items-center gap-4 shadow-sm">
                ...
                <div className="flex justify-between w-full items-center px-4">
                    <div className="flex flex-col items-center">
                        <span className="text-xs font-bold text-blue-500 mb-1">{settings.team1Name}</span>
                    ...
                    <div className="flex flex-col items-center">
                        <span className="text-xs font-bold text-rose-500 mb-1">{settings.team2Name}</span>
                        <span className="text-4xl font-black text-slate-900 dark:text-white tabular-nums">{winner?.t2Final}</span>
                    </div>
                </div>
            </div>

            <div className="w-full space-y-3 max-w-xs px-4">
                <button
                    onClick={() => { setTeam1(t=>({...t, total:0, scores:[]})); setTeam2(t=>({...t, total:0, scores:[]})); setCurrentStep('GAME'); setToast("لعبة جديدة.. حظ موفق! 🔥"); }}
                    className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-950 text-xl font-black py-6 rounded-[2rem] flex items-center justify-center gap-3 active:scale-95 shadow-lg transition-all"
                >
                ...
                <button
                    onClick={() => setCurrentStep('SETUP')}
                    className="w-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-lg font-bold py-4 rounded-[1.5rem] active:scale-95"
                >
                    تغيير الفرق
                </button>
            </div>
